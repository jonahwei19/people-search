"""GET /api/profile/:id — Get a single profile.
POST /api/profile/:id — Update LinkedIn URL + re-enrich."""
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        """Update LinkedIn URL. If URL provided, re-enriches via EnrichLayer."""
        account = require_auth(self)
        if not account:
            return
